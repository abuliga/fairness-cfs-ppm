...
def run_pipeline(CONF):
    ...
    if CONF['predictive_model'] == 'mlp':
        ...
        param_dist = {
            ...
            "xgboost__scale_pos_weight": [1, sum(y_train == 0) / sum(y_train == 1)]
        }
    ...
    if dataset_name == 'bpi2012':
        ...
        event_log.parse_xes_log(CONF['data'])
    else:
    ...
    save_feature_importance(model, train_df, val_df, dataset_name, CONF['fairness_opt'], CONF)
    ...
    def mixed_distance(sol, orig, columns, model, inv_cat_maps):
        ...
        dist = np.linalg.norm(to_dense(sol_trans) - to_dense(orig_trans))
    ...
    def sensitive_attribute_intervention_analysis_inv_maps(max_k=2):
        ...
        for attrs in attr_combos:
            ...
            for idx, x_row in X_test.iterrows():
                x_orig = x_row.copy()
                skip_trace = np.any(x_orig == 'Application Rejected')
...
#    shap.plots.bar(shap_values)
    ...
    for case in ['protected','unprotected']:
        ...
        def fitness_function(ga_instance, solution, solution_idx):
            ...
            return fitness
        found = 0
        for index_trace in range(len(test_cases)):
            ...
            x_original_df = drop_columns(test_cases.iloc[[index_trace]])
            if 'hiring' in dataset_name:
                label_act = 'Application Rejected'
            elif 'bpi2012' in dataset_name:
                label_act = 'A_DECLINED'
            if np.any(x_original == label_act):
                continue
            # Lock sensitive attribute indices to original values
            if 'bpi2012' in dataset_name:
                sensitive_attr = ['prefix_4','prefix_5']
                sensitive_indices = [
            ...
            for i, g in enumerate(gene_space):
                if i in sensitive_indices:
                    if encoded_columns[i] in cat_maps:
                        locked_gene_space.append([int(next(k for k, v in inv_cat_maps[encoded_columns[i]].items() if v == x_original[i]))])
                    else:
            ...
            found += 1
            if found >= 15:
                print("Reached 15 generated traces. Stopping.")
                break
...
if __name__ == "__main__":
    ...
    datasets = ['hiring_log_high-xes.xes','hiring_log_medium-xes.xes','hiring_log_low-xes.xes','bpi2012.xes']
    #datasets = ['bpi2012.xes']
    for dataset in datasets:
        for model in ['mlp']:
            for fairness in [False, True]:
                for enc in [EncodingType.SIMPLE_TRACE.value]:
                    if 'hiring' in dataset:
                        prefix_lengths = [3,4,5,6]
                    elif 'bpi2012' in dataset:
                        prefix_lengths = [3,4,5,6]
                    for prefix in prefix_lengths:
                        CONF = {
                            'data': '/Users/andrei/Desktop/FBK/fairness_cfs/fairness_logs/' + dataset,
                            'train_val_test_split': [0.7, 0.05, 0.25],
